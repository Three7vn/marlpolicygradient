...
from learning.model import Actor, Critic
from learning.multistep_buffer import ReplayBuffer
from learning.hp import SEED, BUFFER_SIZE, BATCH_SIZE, START_SINCE,\
...
class Agent():
    ...
    def estimate_actor_noise_std(self, n=10000, state_size=24, action_size=2):
        """Estimate standard deviation of actor output due to noisy layers.
        
        Args:
            n: Number of random states to test
            state_size: Dimensionality of state space
            action_size: Dimensionality of action space
            
        Returns:
            Standard deviation of actions across random states
        """
        states = torch.randn(n, state_size).to(device)
