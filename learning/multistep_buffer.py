...
Implements a sum-tree data structure for efficient proportional prioritization
and supports multi-step bootstrapping for temporal credit assignment.
"""
...
class ReplayBuffer:
    """Prioritized replay buffer with n-step returns and sum-tree sampling.
    
    Uses a binary sum-tree for O(log n) priority updates and sampling.
    Supports multiple parallel agents and n-step bootstrapping.
    """
    ...
    def __init__(self, n_agents, buffer_size, batch_size, n_multisteps, gamma, a):
        ...
        Args:
            n_agents: Number of parallel agents collecting experiences
            buffer_size: Maximum capacity of the replay buffer
            batch_size: Number of transitions to sample per batch
            n_multisteps: Horizon length for n-step returns
            gamma: Discount factor for future rewards
            a: Priority exponent (alpha) controlling prioritization strength
        """
        ...
        self.gamma = gamma
        self.a = a  # Priority exponent

        self.memory_write_idx = 0  # Current write position in circular buffer
    ...
    def sample(self, beta):
        """Sample a prioritized batch of experiences using proportional sampling.

        Args:
            beta: Importance sampling exponent for bias correction

        Returns:
            experiences: Tuple of (states, actions, rewards, next_states, dones) tensors
            target_discount: Effective discount factor for n-step returns
            is_weights: Importance sampling weights for bias correction
            indices: Array of sampled experience indices for priority updates
        """
        if self.__len__() == 0:
        ...
        while indices_not_prepared:
            ...
            try:
                ...
                # Rare edge case due to floating-point precision during tree traversal
                continue
            else:
    ...
    def update_priorities(self, indices, new_priorities):
        ...
        Args:
            indices: Array of experience buffer indices to update
            new_priorities: New priority values (typically proportional to TD error)
        """
        # Remove duplicate samples (keep only first occurrence)
